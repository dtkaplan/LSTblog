...
date: "2024-03-31"
categories: [correlation, causality]
---
...
> *The problems of ... correlation deal wholly with departures or variations ; they pay no direct regard to the central form from which the departures or variations are measured. If we were measuring statures, and had made a mark on our rule at a height equal to the average height of the race of persons whom we were considering, then it would be the distance of the top of each man's head from that mark, upward or downward as the case might be, that is wanted for our use, and not its distance upward from the ground.*[^465-spurious-correlation-1]

[^465-spurious-correlation-1]: Francis Galton (1890) "Kinship and Correlation" *The North American Review* 150(401) [URL](https://www.jstor.org/stable/25101964)
...
knitr::include_graphics("ngram-correlation2.png")
```
...
> Causation describes a class of hypotheses that DAGs can represent. In that representation, a causal relationship between two nodes X and Y is marked by a causal path connecting X to Y. In Lesson [-@sec-confounding], we defined "causal path" in terms of the directions of arrows in a DAG.[^465-spurious-correlation-2] A definitive demonstration of a causal relationship between X and Y is that intervening to change X results reliably in a change in Y, *all other nodes not on the causal path being held constant.* (Lesson [-@sec-experiment] treats the methodology behind this definitive sign.)

[^465-spurious-correlation-2]: We will consider a "direct causal link" to be a form of causal path.
...
```{r echo=FALSE}
#| column: margin
knitr::include_graphics("correlation-tee-shirt.png")
```

Historically, the rise of the expression "correlation does not imply causation"---@fig-ngram-corr-is-not-cause shows the ngram since the 1888 invention of the correlation coefficient---comes *after* the peak in the use of the word "correlation."

```{r echo=FALSE}
#| label: fig-ngram-corr-is-not-cause2
#| fig-cap: "Google NGram showing the rise in the use of the phrases \"correlation does not imply causation,\" and  \"correlation is not causation\" in recent decades."
#| fig-cap-location: margin
knitr::include_graphics("ngram-correlation2.png")
```
